...
var MapsLib = MapsLib || {}; MapsLib.schemaVersion = 2;
    ...
    MapsLib.fusionTableId = "1Rso12kehUj7JszEAd99fpm4xPNas5I5oDrpOrcny";

    // *New Fusion Tables Requirement* API key. found at https://code.google.com/apis/console/
    // *Important* this key is for demonstration purposes. please register your own.
    MapsLib.googleApiKey ="AIzaSyDmeT5kmTJrgUiVIiUBqVZBaQinFeBZctE";
...
$.extend(MapsLib, {
    ...
    searchPage: { 
        allColumns: false,
        distanceFilter: { 
            entries: [ ["Anywhere", "0", true], ["2 miles"], ["4 miles"], ["6 miles"], ["10 miles"] ]
        },
        columns: [
            { label: "Facility Type", type: "dropdown", foreach: "facility_type",
                entries: [
                    ["Any", "", true],
                ],
             },
            { label: "Facility Name", type: "text", column: "name_facility" }
        ]
    },
    ...
    title: "Nyamira Health Facilities Map",

    // Contents of the About Page.  You can use "{title}" to insert your title.
    aboutPage: " \
        <h3>About {title}</h3> \
        <p>This is a demonstration of an interactive Mobile and Web-based map. Code written by ~Douglas Mbura as proposal for implementation by DRSRS.  For details contact ~Gordon Ojwang (gordonojwang@gmail.com). </p> \
        ",
    ...
    customInfoboxHtml: ' \
        ...
        {{/if}} \
        <div class="entity blue_box {{row.Grantee_Organization_Type_Description}}"><span id="entity-text">H</span></div> \
        <div class="entity red_box {{row.Grantee_Organization_Type_Description}}"><span id="entity-text">H</span></div> \
        <div class="entity orange_box {{row.Grantee_Organization_Type_Description}}"><span id="entity-text">H</span></div> \
        <h4 class="infobox-header">{{row.name_facility}}</h4> \
        {{#if isListView}} \
            <p class="ui-li-desc infobox-subheader"> \
            {{row.year_established}}<br> \
            {{row.facility_type}}<br> \
            {{row.managing_authority}}</p> \
        {{else}} \
            <p></p><p class="ui-li-desc"> \
            {{row.offered_services}}<br> \
            {{row.key_strengths}}<br> \
            {{row.Address}}<br> \
            {{#if row.URL}} \
                <a href="{{row.URL}}" target="_blank">{{row.URL}}</a><br> \
            {{/if}} \
            <a href="tel:1{{row.Telephone_Number}}">{{row.email_address}}</a></p> \
        {{/if}} \
        </p></div>',
    ...
    useNearbyLocation: {
        ...
        boundsExceededMessage:      "You're currently outside Nyamira County. Map will auto-zoom to nyamira.",
